...
- НЕ реализовано:
  ...
  - проблемы с get ingredient, get recipe, get link
  - Импорты модели пользователя делать через settings.AUTH_USER_MODEL и get_user_model()

  - Ограничения на значения полей
  - структура регистрация  +, ингридиентыЛистФильтр  ?, подпискиЛист  +
  - Кастомная пагинация
  - аватар PUT, 400  +
  - ингредиенты GET id ?
  - Опечатка в имени переменной firstIndredientId
  - фильтрация рецепты по автору ?
  - создание рецепта с несуществующим ингредиетном 400  +
  - валидация из моделей в сериализаторы  +?
  - фильтрация есть в корзине, в избранном  ?
  - в тестах проблема с {{параметрами}}, надо либо замерить на существующие значения, либо читать, как это чинится

- 18 запуска коллекции осталось.
